...
struct PatientMedicalRecordView: View {
    ...
    var searchResults: [MedicalHistoryItem] {
            if searchText.isEmpty {
                return globalDataModel.medicalHistory
            } else {
                return globalDataModel.medicalHistory.filter { $0.diagnostic.localizedCaseInsensitiveContains(searchText) }
            }
        }
    ...
    var body: some View {
        NavigationView{
            List{
                ForEach(globalDataModel.medicalHistory) { item in
                    ...
                    .searchable(text: $searchText, prompt: "Buscar registro")
                    .toolbar{
